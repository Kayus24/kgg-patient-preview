const CACHE_NAME = 'kgg-patient-preview-patient-ticket-034-modal-counter-20260910-1635-r1-v82';
...
function isIndexRequest(request){const url=new URL(request.url);if(url.origin!==self.location.origin)return false;const scopePath=new URL(self.registration.scope).pathname;return url.pathname===scopePath||url.pathname===scopePath+'index.html'}
function isRecoveryRequest(request){const url=new URL(request.url);if(url.origin!==self.location.origin)return false;const scopePath=new URL(self.registration.scope).pathname;return url.pathname===scopePath+'update-recovery.html'}
