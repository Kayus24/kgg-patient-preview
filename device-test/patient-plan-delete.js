(()=>{
  ...
  const isEn=()=>window.KGGDeviceTestStorage.getItem('kggPatientLang')==='en';
  const t=(de,en)=>isEn()?en:de;
  const esc=s=>String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const clone=v=>{try{return JSON.parse(JSON.stringify(v))}catch(e){return v}};
  const safe=fn=>{try{return fn()}catch(e){return null}};

  function readState(){try{return JSON.parse(window.KGGDeviceTestStorage.getItem(MULTI_KEY)||'null')}catch(e){return null}}
  function writeState(state){state.updatedAt=new Date().toISOString();window.KGGDeviceTestStorage.setItem(MULTI_KEY,JSON.stringify(state))}
  function persistCurrent(raw){window.KGGDeviceTestStorage.setItem(CURRENT_KEY,JSON.stringify({plan:raw,importedAt:new Date().toISOString()}))}
  ...
  function removeLocalPlanKeys(raw){planStorageKeys(raw).forEach(key=>window.KGGDeviceTestStorage.removeItem(key))}
  function migratePlanKeys(oldRaw,newRaw){const oldKeys=planStorageKeys(oldRaw||{}),newKeys=planStorageKeys(newRaw||{});oldKeys.forEach((oldKey,index)=>{const newKey=newKeys[index];if(oldKey===newKey)return;const value=safe(()=>window.KGGDeviceTestStorage.getItem(oldKey));if(value!==null&&window.KGGDeviceTestStorage.getItem(newKey)===null)window.KGGDeviceTestStorage.setItem(newKey,value);if(value!==null)window.KGGDeviceTestStorage.removeItem(oldKey)})}
