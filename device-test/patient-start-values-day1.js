(()=>{
  ...
  function applyStartValues(){
    ...
    try{
      ...
      if(window.KGGDeviceTestStorage.getItem(marker))return false;
      if(hasPatientProgress()){window.KGGDeviceTestStorage.setItem(marker,'skipped-existing-progress');return false}
      let wrote=0;
      (p.ex||[]).forEach((e,ei)=>{
      ...
      if(!wrote){window.KGGDeviceTestStorage.setItem(marker,'no-empty-targets');return false}
      if(!done.includes(1))done.push(1);
      if(p.extendDays!==false||Number(p.days||0)>=2)d=2;else d=1;
      save();
      window.KGGDeviceTestStorage.setItem(marker,new Date().toISOString());
