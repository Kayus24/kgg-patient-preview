(()=>{
  ...
  const isEn=()=>window.KGGDeviceTestStorage.getItem(LANG_KEY)==='en';
  ...
  function storedCurrentPlan(){const saved=safe(()=>JSON.parse(window.KGGDeviceTestStorage.getItem(CURRENT_KEY)||'null'));return saved&&saved.plan&&typeof saved.plan==='object'?saved.plan:null}
  ...
  function writeState(s){s.updatedAt=new Date().toISOString();window.KGGDeviceTestStorage.setItem(MULTI_KEY,JSON.stringify(s))}
