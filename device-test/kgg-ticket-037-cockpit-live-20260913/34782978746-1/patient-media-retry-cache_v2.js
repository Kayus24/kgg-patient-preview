(()=>{
  ...
  function storedPlan(){const saved=safe(()=>JSON.parse(window.KGGDeviceTestStorage.getItem(PLAN_KEY)||'null'));return saved&&saved.plan?saved.plan:null;}
  ...
  function setBox(id,html,cls){const box=mediaBox(id);if(!box)return;box.className=(box.getAttribute('data-kgg-progression-media')?'kggProgressionMediaBox ':'kggMediaBox ')+(cls||'');box.innerHTML=html;if(cls==='ready'||box.querySelector('img'))setTimeout(()=>syncThumbById(id),0);}
