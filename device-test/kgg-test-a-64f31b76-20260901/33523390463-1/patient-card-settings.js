(()=>{
  ...
  const en=()=>window.KGGDeviceTestStorage.getItem(LANG)==='en';
  const T=(de,enText)=>en()?enText:de;
  const safe=f=>{try{return f()}catch(e){return null}};
  const has=(o,k)=>Object.prototype.hasOwnProperty.call(o||{},k);

  function readSettings(){try{return JSON.parse(window.KGGDeviceTestStorage.getItem(SET_KEY)||'{}')||{}}catch(e){return{}}}
  function writeSettings(s){window.KGGDeviceTestStorage.setItem(SET_KEY,JSON.stringify(s||{}))}
  ...
  function storePlan(){safe(()=>window.KGGDeviceTestStorage.setItem(PLAN_KEY,JSON.stringify({plan:rawPlan(),importedAt:new Date().toISOString()})))}
