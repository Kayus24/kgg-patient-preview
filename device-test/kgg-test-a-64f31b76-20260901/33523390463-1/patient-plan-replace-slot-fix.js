(()=>{
  ...
  const nativeSetItem=window.KGGDeviceTestStorage.setItem;
  ...
  function existingPlanCount(){
    const state=parse(window.KGGDeviceTestStorage&&window.KGGDeviceTestStorage.getItem(MULTI_KEY));
    if(state&&Array.isArray(state.plans)&&state.plans.length)return state.plans.length;
    const current=parse(window.KGGDeviceTestStorage&&window.KGGDeviceTestStorage.getItem(CURRENT_KEY));
  ...
  function isEnglish(){
    try{return window.KGGDeviceTestStorage.getItem('kggPatientLang')==='en'}catch(e){return false}
  }
  function currentRaw(){
    const saved=parse(window.KGGDeviceTestStorage&&window.KGGDeviceTestStorage.getItem(CURRENT_KEY));
  ...
  function writeAddedCurrent(key){
    ...
    nativeSetItem.call(window.KGGDeviceTestStorage,key,JSON.stringify(wrapper));
  ...
  function finishAdd(key){
    ...
    nativeSetItem.call(window.KGGDeviceTestStorage,key,JSON.stringify(state));
    addSession=null;
    queueRefresh();
    return true;
  }
  ...
  window.KGGDeviceTestStorage.setItem=function(key,value){
    const name=String(key);
    if(this===window.KGGDeviceTestStorage&&name===CURRENT_KEY&&addSession&&addSession.captured){
      writeAddedCurrent(key);
      return;
    }
    if(this===window.KGGDeviceTestStorage&&name===MULTI_KEY){
    ...
    if(this===window.KGGDeviceTestStorage&&(name===CURRENT_KEY||name===MULTI_KEY))queueRefresh();
    return result;
  };
