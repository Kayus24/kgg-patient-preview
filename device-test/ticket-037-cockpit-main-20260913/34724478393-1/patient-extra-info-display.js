(()=>{
  ...
  function rawPlan(){try{let h=location.hash.slice(1);if(h.startsWith('KGGH2:'))return JSON.parse(dec(h.slice(6)))}catch(e){}try{let x=JSON.parse(window.KGGDeviceTestStorage.getItem(KEY)||'null');return x&&x.plan?x.plan:null}catch(e){return null}}
  ...
  function fromArr(a){if(!Array.isArray(a))return fromObj(a);let m=a[7];let vals=[fromObj(m),a[11],a[12],a[13]];for(const v of vals){if(typeof v!=='string')continue;let x=clean(v);if(x)return x}return''}
